...
class TestDatabase(unittest.TestCase):
    def setUp(self):
        ...
        self.engine = sqlalchemy.create_engine('mysql+pymysql://root:1234@localhost:3306/dw')
        ...
        ORDER BY ss.id ASC
        """
        result = self.connection.execute(text(query)) # Izvršavanje upita
        self.db_df = pd.DataFrame(result.fetchall()) # Dohvaćanje rezultata upita
        self.db_df.columns = list(result.keys()) # Dohvaćanje naziva stupaca
